using ProjectHeyMobile.Authentication.Util;
using System;
...
namespace ProjectHeyMobile.Authentication.Signers
{
    /// <summary>
    ...
    public abstract class AWS4SignerBase
    {
        ...
        protected byte[] DeriveSigningKey(string awsSecretAccessKey, string region, string date, string service)
        {
            ...
            byte[] hashDate = HMACSHA256ComputeKeyedHash(Encoding.UTF8.GetBytes(ksecret), Encoding.UTF8.GetBytes(date));
            byte[] hashRegion = HMACSHA256ComputeKeyedHash(hashDate, Encoding.UTF8.GetBytes(region));
            byte[] hashService = HMACSHA256ComputeKeyedHash(hashRegion, Encoding.UTF8.GetBytes(service));
            return HMACSHA256ComputeKeyedHash(hashService, Encoding.UTF8.GetBytes(TERMINATOR));
        }
        ...
        protected byte[] HMACSHA256ComputeKeyedHash(byte[] key, byte[] data)
        {
